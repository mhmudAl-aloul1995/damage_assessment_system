    ...
    private function dashboardCoreStats(Request $request): array
    {
        ...
        $buildings = $buildingQuery
            ->selectRaw("COALESCE(SUM(field_status = 'Not_Completed'), 0) as not_completed,
                ...
                COALESCE(SUM(CASE WHEN LOWER(TRIM(COALESCE(building_damage_status, ''))) IN ('fully_damaged', 'fully_damaged2', 'totally_damaged', 'total_damage', 'totally') THEN 1 ELSE 0 END), 0) as fully_damaged,
                COALESCE(SUM(CASE WHEN LOWER(TRIM(COALESCE(building_damage_status, ''))) IN ('partially_damaged', 'partially_damaged2', 'partial_damage', 'partial') THEN 1 ELSE 0 END), 0) as partially_damaged,
                COALESCE(SUM(CASE WHEN LOWER(TRIM(COALESCE(building_damage_status, ''))) IN ('committee_review', 'committee_review2', 'commite_review') THEN 1 ELSE 0 END), 0) as committee_review,
                COALESCE(SUM(CASE WHEN LOWER(TRIM(COALESCE(building_damage_status, ''))) IN ('no_damage', 'no_damage2', 'no_damaged') THEN 1 ELSE 0 END), 0) as no_damage,
