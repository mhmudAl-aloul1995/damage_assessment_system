<div class="row g-6 g-xl-9" id="engineers-container">
    @foreach ($engineers as $value)
        <div class="col-md-6 col-xl-4 assessment-card">
            ...
            <a href="{{ url('damage-assessment/assessment/' . $value->globalid) }}" 
               ...
               style="text-decoration: none; transition: all 0.3s ease;">
                ...
                <div class="card-header border-0 pt-7">
                    ...
                    <div class="card-toolbar d-flex gap-2">
                        <!-- حالة الإستبيان -->
                        <span class="badge badge-light-{{$value->field_status == 'COMPLETED' ? 'success' : 'warning'}} fw-bold px-3 py-2">
                            {{ $value->field_status == 'COMPLETED' ? 'مكتمل' : 'قيد التدقيق' }}
                        </span>
                        ...
                        @endphp
                        <span class="badge badge-{{$damageClass}} fw-bold px-3 py-2">
                ...
                <div class="card-body p-9">
                    ...
                    <div class="fs-3 fw-bolder text-gray-900 mb-1">
                        {{ $value->building_name ?? $value->owner_name ?? 'بدون اسم' }}
                    </div>

                    <div class="text-gray-500 fw-bold fs-6 mb-6">
                        <span class="text-primary"> • رقم المبنى</span>#{{$value->objectid}}
                    </div>
                    ...
                    <div class="d-flex flex-wrap mb-7">
                        <div class="border border-gray-300 border-dashed rounded min-w-100px py-2 px-3 me-3 mb-2">
                            ...
                            <div class="fs-7 text-gray-800 fw-bolder">{{ $value->date_of_damage ?? '-' }}</div>
                        </div>
                        ...
                        <div class="border border-gray-300 border-dashed rounded min-w-100px py-2 px-3 mb-2">
                            ...
                            <div class="fs-7 text-gray-800 fw-bolder text-center">{{$value->damaged_units_nos}}</div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex align-items-center">
                        <div class="symbol-group symbol-hover mb-1">
                            @foreach ($value->housing_unit->take(4) as $hou)
                                <div class="symbol symbol-35px symbol-circle" 
                                     data-bs-toggle="tooltip" 
                                     title="{{ $hou->q_9_3_1_first_name }} {{ $hou->q_13_3_4_last_name__family }}">
                                    <span class="symbol-label bg-light-success text-success fw-bold fs-8 border border-white">
                                        {{ substr($hou->q_9_3_1_first_name, 0, 2) }}
                                    </span>
                                </div>
                            @endforeach
                            
                            @if($value->housing_unit->count() > 4)
                                <div class="symbol symbol-35px symbol-circle">
                                    <span class="symbol-label bg-light-dark text-gray-800 fw-bold fs-8">
                                        +{{ $value->housing_unit->count() - 4 }}
                                    </span>
                                </div>
                            @endif
                        </div>
...
<div class="d-flex flex-stack flex-wrap pt-10">
    <div class="fs-6 fw-bold text-gray-600">
        عرض <span class="text-gray-900">{{ $engineers->firstItem() }}</span> إلى <span class="text-gray-900">{{ $engineers->lastItem() }}</span> من أصل <span class="text-primary">{{ $engineers->total() }}</span>
    </div>
