@extends('layouts.app')
@section('title', 'الباحثين')
@section('pageName', 'الباحثين')
		...
		var KTEngineersList = function () {
			...
			var initEngineerTable = function () {
				...
				datatable = $(table).DataTable({
					serverSide: true,
					ajax: {
						url: "{{url('damage-assessment/engineer/show')}}",
						data: function (d) {
					...
					createdRow: (row, data, index) => {
						...
						$(row).on('click', function (e) {
							e.preventDefault()
							var url_eng="{{url('damage-assessment/engineerAssessments')  }}/"+data.assignedto
