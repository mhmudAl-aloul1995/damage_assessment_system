        ...
        <div class="card-body"><div class="table-responsive">
            <table class="table table-rounded table-striped align-middle table-row-dashed fs-6 gy-5 w-100" id="kt_table_Housing">
                <thead><tr class="text-start text-muted fw-bold border-bottom border-gray-200 fs-7 text-uppercase gs-0">
                    <th>{{ __('ui.housing_page.researcher') }}</th><th>{{ __('ui.housing_page.building_number') }}</th><th>{{ __('ui.housing_page.object_id') }}</th><th>{{ __('ui.housing_page.unit_number') }}</th><th>{{ __('ui.housing_page.unit_owner') }}</th><th>{{ __('ui.housing_page.damage_status') }}</th><th>{{ __('ui.housing_page.floor_number') }}</th><th>{{ __('ui.housing_page.damaged_area') }}</th><th>{{ __('ui.housing_page.municipality') }}</th><th>{{ __('ui.housing_page.neighborhood') }}</th><th>{{ __('ui.housing_page.support_summary') }}</th><th>{{ __('ui.housing_page.updated_at') }}</th><th class="text-end">{{ __('ui.housing_page.action') }}</th>
                </tr></thead>
    ...
    <script>
        var KTHousingList = function () {
            ...
            const initHousingTable = function () {
                if (!table) return;
                datatable = $(table).DataTable({ serverSide: true, processing: true, pageLength: 10, order: [[11, 'desc']], ajax: { url: "{{ url('damage-assessment/housing/show') }}", data: function (d) { d.filters = filterPayload(); d.parentglobalid = parentGlobalId; } }, columns: [
