<?php

declare(strict_types=1);

namespace App\Http\Requests\RoadFacility;

use Illuminate\Foundation\Http\FormRequest;

class RoadFacilityFilterRequest extends FormRequest
{
    public function authorize(): bool
    {
        return true;
    }

    public function rules(): array
    {
        return [
            'municipalitie' => ['nullable'],
            'municipalitie.*' => ['nullable', 'string', 'max:255'],
            'neighborhood' => ['nullable'],
            'neighborhood.*' => ['nullable', 'string', 'max:255'],
            'assignedto' => ['nullable'],
            'assignedto.*' => ['nullable', 'string', 'max:255'],
            'assignedto' => ['nullable'],
            'assignedto.*' => ['nullable', 'string', 'max:255'],
            'filters' => ['nullable', 'array'],
            'filters.*' => ['nullable'],
            'filters.*.*' => ['nullable', 'string', 'max:255'],
            'from_date' => ['nullable', 'date'],
            'to_date' => ['nullable', 'date', 'after_or_equal:from_date'],
            'damaged_only' => ['nullable', 'boolean'],
            'undamaged_only' => ['nullable', 'boolean'],
            'security_situation' => ['nullable', 'string', 'max:255'],
            'with_items' => ['nullable', 'boolean'],
            'has_municipality' => ['nullable', 'boolean'],
            'has_neighborhood' => ['nullable', 'boolean'],
            'potholes_only' => ['nullable', 'boolean'],
            'obstacles_only' => ['nullable', 'boolean'],
            'buried_bodies_only' => ['nullable', 'boolean'],
            'uxo_only' => ['nullable', 'boolean'],
            'search' => ['nullable'],
            'search.value' => ['nullable', 'string', 'max:255'],
            'road_facility_columns' => ['nullable', 'array'],
            'road_facility_columns.*' => ['nullable', 'string', 'max:255'],
            'road_facility_columns_mode' => ['nullable', 'string', 'in:except'],
            'road_facility_columns_excluded' => ['nullable', 'array'],
            'road_facility_columns_excluded.*' => ['nullable', 'string', 'max:255'],
            'road_facility_item_columns' => ['nullable', 'array'],
            'road_facility_item_columns.*' => ['nullable', 'string', 'max:255'],
            'road_facility_item_columns_mode' => ['nullable', 'string', 'in:except'],
            'road_facility_item_columns_excluded' => ['nullable', 'array'],
            'road_facility_item_columns_excluded.*' => ['nullable', 'string', 'max:255'],
            'draw' => ['nullable'],
            'start' => ['nullable'],
            'length' => ['nullable'],
        ];
    }

    public function messages(): array
    {
        return [
            'to_date.after_or_equal' => 'The end date must be after or equal to the start date.',
        ];
    }
}
