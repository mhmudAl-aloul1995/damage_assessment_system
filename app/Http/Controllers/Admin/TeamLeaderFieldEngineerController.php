...
class TeamLeaderFieldEngineerController extends Controller
{
    ...
    {
        $request->merge([
            'field_engineer_id' => array_values(array_filter((array) $request->input('field_engineer_id', []))),
        ]);
        ...
        $existingFieldEngineerIds = TeamLeaderFieldEngineer::query()
            ->where('team_leader_id', $teamLeader->id)
            ->whereIn('field_engineer_id', $request->field_engineer_id)
            ->pluck('field_engineer_id');

        if ($existingFieldEngineerIds->isNotEmpty()) {
            throw ValidationException::withMessages([
                'field_engineer_id' => 'يوجد ربط مسبق لنفس قائد الفريق مع أحد المهندسين المحددين.',
            ]);
        }
