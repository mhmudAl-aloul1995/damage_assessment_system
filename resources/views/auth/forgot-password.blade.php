...
<head>
    <base href="../../../" />
    <title>{{ __('ui.auth.forgot_password') }} - {{ __('ui.app.name') }}</title>
    <meta charset="utf-8" />
                    ...
                    <div class="d-flex flex-center flex-column flex-column-fluid px-lg-10 pb-15 pb-lg-20">
                        <form class="form w-100" method="POST" action="{{ route('password.email') }}">
                            ...
                            <div class="text-center mb-11">
                                <h1 class="text-dark fw-bolder mb-3">{{ __('ui.auth.forgot_password') }}</h1>
                                <div class="text-gray-500 fw-semibold fs-6">{{ __('ui.auth.password_reset_hint') }}</div>
                            </div>
                            ...
                            <div class="fv-row mb-8">
                                <input type="email" placeholder="{{ __('ui.auth.email') }}" name="email" value="{{ old('email') }}"
                                    autocomplete="email" class="form-control bg-transparent @error('email') is-invalid @enderror" required autofocus />
                                @error('email')
                            ...
                            <div class="d-grid mb-10">
                                <button type="submit" class="btn btn-primary">{{ __('ui.auth.email_password_reset_link') }}</button>
                            </div>

                            <div class="text-center">
                                <a href="{{ route('login') }}" class="link-primary">{{ __('ui.auth.back_to_login') }}</a>
                            </div>
                        </form>
                    </div>
                </div>
