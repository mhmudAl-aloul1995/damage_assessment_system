...
@section('content')
<style>
    .building-status-btn,.housing-status-btn{transition:all .2s ease-in-out}
    .building-status-btn:hover,.housing-status-btn:hover{transform:translateY(-1px)}
    .building-status-btn.is-active,.housing-status-btn.is-active{transform:translateY(-1px);box-shadow:0 .5rem 1rem rgba(0,0,0,.12)}
    .building-status-btn.btn-light-danger.is-active,.housing-status-btn.btn-light-danger.is-active{background:var(--bs-danger)!important;border-color:var(--bs-danger)!important;color:#fff!important}
    .building-status-btn.btn-light-success.is-active,.housing-status-btn.btn-light-success.is-active{background:var(--bs-success)!important;border-color:var(--bs-success)!important;color:#fff!important}
    .building-status-btn.btn-light-warning.is-active,.housing-status-btn.btn-light-warning.is-active{background:var(--bs-warning)!important;border-color:var(--bs-warning)!important;color:#fff!important}
    .building-status-btn:disabled,.housing-status-btn:disabled{cursor:not-allowed;opacity:.8}

    #housing_table tbody tr.selected{background-color:rgba(0,158,247,.12)!important}
    .container-loader{display:none!important}

    .audit-sticky-menu{position:sticky;top:95px;z-index:10;border-radius:1rem;overflow:hidden}
    .summary-box{min-height:78px;border-radius:.85rem;padding:.85rem;display:flex;flex-direction:column;justify-content:center;transition:all .2s ease}
    .summary-box:hover{transform:translateY(-2px);box-shadow:0 .5rem 1rem rgba(0,0,0,.08)}
    .summary-title{font-size:1rem;font-weight:700;color:var(--bs-gray-700);margin-bottom:.35rem;line-height:1.4}
    .summary-value{font-size:1.2rem;font-weight:800;line-height:1.2;word-break:break-word}

    .audit-toolbar-sticky{position:sticky;top:70px;z-index:30;background:#fff;padding:.75rem 0;border-bottom:1px solid #eef1f5}
    .audit-filter-btn.is-active{background:var(--bs-primary)!important;color:#fff!important}

    .assessment-section{border:1px solid #edf1f5;border-radius:1rem;overflow:hidden;background:#fff;box-shadow:0 .35rem 1rem rgba(15,23,42,.04)}
    .assessment-section-header{cursor:pointer;padding:1rem 1.25rem;border:0;background:linear-gradient(135deg,#f8fafc 0%,#eef6ff 45%,#e8f3ff 100%);border-bottom:1px solid #e4ecf7;transition:all .25s ease;position:relative}
    .assessment-section-header:hover{background:linear-gradient(135deg,#eef6ff 0%,#dceeff 100%);box-shadow:inset 0 0 0 1px rgba(0,158,247,.08)}
    .assessment-section-header .fw-bold{color:#1e2b3b;font-size:1.02rem;letter-spacing:.2px}
    .assessment-section-header .badge{background:#fff;color:#009ef7;border:1px solid rgba(0,158,247,.18);font-weight:700;padding:.45rem .75rem;border-radius:.7rem}
    .assessment-section-header:after{content:"";position:absolute;left:0;top:0;bottom:0;width:4px;background:linear-gradient(180deg,#009ef7,#50cd89)}

    .section-progress-bar{width:120px;height:6px;border-radius:20px;background:#eef3f7;overflow:hidden}
    .section-progress-fill{height:100%;border-radius:20px;background:linear-gradient(90deg,#009ef7,#50cd89)}
    .assessment-item{border-top:1px dashed var(--bs-gray-300);padding:1rem 1.25rem;background:#fff}
    .assessment-item.has-answer{background:#f1fff5}
    .assessment-item.is-missing{background:#fff5f8!important;border-right:4px solid #f1416c}
    .assessment-item.is-edited{border-right:4px solid #009ef7}
    .assessment-item:hover{background:#f8fbff}
    .assessment-question{font-weight:800;color:var(--bs-gray-800);line-height:1.6}
    .assessment-answer{font-weight:700;color:var(--bs-gray-700);word-break:break-word}
    .assessment-edit .select2-container,.assessment-edit select,.assessment-edit input{width:100%!important}
    .assessment-item img{max-width:90px;height:90px;object-fit:cover;border-radius:.75rem;margin:3px}
    .assessment-progress{min-width:90px}

    .audit-save-indicator{position:fixed;bottom:25px;left:25px;z-index:9999;display:none;padding:.75rem 1rem;border-radius:.75rem;background:#50cd89;color:#fff;font-weight:800;box-shadow:0 .75rem 1.5rem rgba(0,0,0,.15)}

    @media(max-width:991px){
        .audit-sticky-menu{position:relative;top:auto;margin-bottom:1rem}
        .summary-box{min-height:70px;padding:.75rem}
        .summary-title{font-size:.72rem}
        .summary-value{font-size:1rem}
    }
    @media(max-width:768px){
        .assessment-section-header{padding:.9rem 1rem}
        .assessment-section-header .fw-bold{font-size:.92rem;line-height:1.5}
        .assessment-section-header .badge{font-size:.72rem;padding:.35rem .55rem}
        .audit-toolbar-sticky{top:60px}
    }
</style>

<div class="card card-flush mb-7">
    <div class="card-header pt-7">
        <div class="card-title"><h2>الإستبيان</h2></div>
    </div>
                    ...
                    <div class="card-header border-0 pt-6 pb-4">
                        <div class="card-title">
                            <div class="d-flex align-items-center position-relative my-1">
                                ...
                                <input type="text" data-kt-buildingAssessment-table-filter="search" class="form-control form-control-solid w-250px ps-13" placeholder="بحث" />
                            </div>
                        </div>
                        ...
                        <div class="card-toolbar">
                            <div class="d-flex justify-content-end align-items-center gap-2 flex-wrap">
                                @role('Legal Auditor')
                                <button type="button" class="btn btn-sm btn-light-success building-status-btn" data-status="accepted" onclick="setBuildingStatus('accepted')">مقبول</button>
                                <button type="button" class="btn btn-sm btn-light-warning building-status-btn" data-status="legal_notes" onclick="setBuildingStatus('legal_notes')">ملاحظات قانونية</button>
                                @endrole

                                @role('QC/QA Engineer')
                                <button type="button" class="btn btn-sm btn-light-danger building-status-btn" data-status="rejected" onclick="setBuildingStatus('rejected')">مرفوض</button>
                                <button type="button" class="btn btn-sm btn-light-success building-status-btn" data-status="accepted" onclick="setBuildingStatus('accepted')">مقبول</button>
                                <button type="button" class="btn btn-sm btn-light-warning building-status-btn" data-status="need_review" onclick="setBuildingStatus('need_review')">بحاجة لمراجعة</button>
                                @endrole

                                <button type="button" class="btn btn-sm btn-light-dark" onclick="openNotesModal('building','history')">ملاحظات</button>
                                <button type="button" class="btn btn-sm btn-light-info" onclick="openNotesModal('building','edit_note')">تعديل الملاحظة</button>
                                <button type="button" class="btn btn-sm btn-light-primary ms-3" onclick="reloadBuildingAssessmentTable()">تحديث</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body pt-0 pb-4">
                        <div class="audit-toolbar-sticky mb-4">
                            <div class="d-flex flex-wrap gap-2">
                                <button type="button" class="btn btn-sm btn-light-primary audit-filter-btn building-filter-btn is-active" data-filter="all">الكل</button>
                                <button type="button" class="btn btn-sm btn-light-danger audit-filter-btn building-filter-btn" data-filter="missing">الفارغ فقط</button>
                                <button type="button" class="btn btn-sm btn-light-info audit-filter-btn building-filter-btn" data-filter="edited">المعدّل فقط</button>
                                <button type="button" class="btn btn-sm btn-light-success audit-filter-btn building-filter-btn" data-filter="answered">المجاب فقط</button>
                            </div>
                        </div>
                        ...
                        <table class="d-none" id="kt_table_building_assessment">
                            <thead><tr><th>السؤال</th><th>الجواب</th><th>تعديل الإجابة</th></tr></thead>
                            <tbody></tbody>
                        </table>
                    </div>
                </div>
            ...
            <div class="tab-pane fade" id="tab_housing" role="tabpanel">
                <div class="card card-flush mb-7 shadow-sm border-0">
                    <div class="card-header pt-6 pb-4 border-0">
                        <div class="card-title"><h3 class="fw-bold mb-0">وحدات المبنى</h3></div>
                        <div class="card-toolbar">
                            <button type="button" class="btn btn-sm btn-light-primary" onclick="reloadBuildingUnitsTable()">
                    ...
                    <div class="card-body pt-0 pb-4">
                        <div class="table-responsive">
                            <table class="table align-middle table-row-bordered table-rounded gs-7 gy-4" id="housing_table">
                                <thead>
                    ...
                    <div class="col-12 col-lg-3 col-xl-2">
                        <div class="card card-flush shadow-sm border-0 audit-sticky-menu">
                            <div class="card-header py-3 px-4">
                                <div class="card-title m-0"><h3 class="fw-bold fs-4 mb-0">ملخص الوحدة</h3></div>
                            </div>

                            <div class="card-body p-3">
                                <div class="row g-3">
                                    <div class="col-6 col-lg-12"><div class="summary-box bg-light-primary"><div class="summary-title">مساحة الوحدة</div><div id="sidebar_unit_area" class="summary-value text-primary">--</div></div></div>
                                    <div class="col-6 col-lg-12"><div class="summary-box bg-light-warning"><div class="summary-title">تأهيل مطبخ</div><div id="sidebar_kitchen" class="summary-value text-warning">--</div></div></div>
                                    <div class="col-6 col-lg-12"><div class="summary-box bg-light-warning"><div class="summary-title">تأهيل حمام</div><div id="sidebar_bathroom" class="summary-value text-warning">--</div></div></div>
                                    <div class="col-6 col-lg-12"><div class="summary-box bg-light-success"><div class="summary-title">ملائمة للسكن</div><div id="sidebar_living" class="summary-value text-success">--</div></div></div>
                                    <div class="col-12 col-lg-12"><div class="summary-box bg-light-info"><div class="summary-title">عدد الغرف</div><div id="sidebar_rooms" class="summary-value text-info">--</div></div></div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card card-flush shadow-sm border-0">
                            <div class="card-header border-0 pt-6 pb-4">
                                <div class="card-title">
                                    <div class="d-flex align-items-center position-relative my-1">
                                        <i class="ki-duotone ki-magnifier fs-3 position-absolute ms-5"><span class="path1"></span><span class="path2"></span></i>
                                        <input type="text" data-kt-HousingAssessment-table-filter="search" class="form-control form-control-solid w-250px ps-13" placeholder="بحث" />
                                    </div>
                                </div>
                                ...
                                <div class="card-toolbar">
                                    <div class="d-flex align-items-center flex-wrap fw-bold gap-2">
                                        <div class="me-3">
                                            <select name="globalid" class="form-select form-select-solid w-250px" data-control="select2" data-placeholder="إختر الوحدة">
                                                <option value=""></option>
                                        ...
                                        @role('Legal Auditor')
                                        <button type="button" class="btn btn-sm btn-light-success housing-status-btn" data-status="accepted" onclick="setHousingStatus('accepted')">مقبول</button>
                                        <button type="button" class="btn btn-sm btn-light-warning housing-status-btn" data-status="legal_notes" onclick="setHousingStatus('legal_notes')">بحاجة لمراجعة</button>
                                        @endrole

                                        @role('QC/QA Engineer')
                                        <button type="button" class="btn btn-sm btn-light-danger housing-status-btn" data-status="rejected" onclick="setHousingStatus('rejected')">مرفوض</button>
                                        <button type="button" class="btn btn-sm btn-light-success housing-status-btn" data-status="accepted" onclick="setHousingStatus('accepted')">مقبول</button>
                                        <button type="button" class="btn btn-sm btn-light-warning housing-status-btn" data-status="need_review" onclick="setHousingStatus('need_review')">بحاجة لمراجعة</button>
                                        @endrole

                                        <button type="button" class="btn btn-sm btn-light-dark" onclick="openNotesModal('housing','history')">ملاحظات</button>
                                        <button type="button" class="btn btn-sm btn-light-info" onclick="openNotesModal('housing','edit_note')">تعديل الملاحظة</button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card-body pt-0 pb-4">
                                <div class="audit-toolbar-sticky mb-4">
                                    <div class="d-flex flex-wrap gap-2">
                                        <button type="button" class="btn btn-sm btn-light-primary audit-filter-btn housing-filter-btn is-active" data-filter="all">الكل</button>
                                        <button type="button" class="btn btn-sm btn-light-danger audit-filter-btn housing-filter-btn" data-filter="missing">الفارغ فقط</button>
                                        <button type="button" class="btn btn-sm btn-light-info audit-filter-btn housing-filter-btn" data-filter="edited">المعدّل فقط</button>
                                        <button type="button" class="btn btn-sm btn-light-success audit-filter-btn housing-filter-btn" data-filter="answered">المجاب فقط</button>
                                        <button type="button" class="btn btn-sm btn-light-warning audit-filter-btn housing-filter-btn" data-filter="attachments">المرفقات</button>
                                    </div>
                                </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div id="notesInputWrapper" style="display:none;">
                    ...
                    <textarea id="notesInput" class="form-control form-control-solid" rows="5" placeholder="اكتب الملاحظة هنا..."></textarea>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="notesSaveBtn" style="display:none;" onclick="submitStatusWithNotes()">حفظ</button>
            </div>
        </div>
    </div>
</div>
...
const BUILDING_SURVEY_MAP = {
    ...
    date_of_damage: ['1. Building Information', 104],
    building_material: ['1. Building Information', 105],
    other_material: ['1. Building Information', 106],
    building_age: ['1. Building Information', 107],
    land_area: ['1. Building Information', 108],
    floor_nos: ['1. Building Information', 109],
    ground_floor_area__m2: ['1. Building Information', 110],
    floor_area_m2: ['1. Building Information', 111],
...
const HOUSING_SURVEY_MAP = {
    photos_final_comments_unilt: ['1. Attachments & Final Comments', 1],
    damge_photo_1: ['1. Attachments & Final Comments', 2],
    damge_photo_2: ['1. Attachments & Final Comments', 3],
    damge_photo_3: ['1. Attachments & Final Comments', 4],
    damge_photo_4: ['1. Attachments & Final Comments', 5],
    final_comments: ['1. Attachments & Final Comments', 6],
...
var KTBuildingAssessmentList = function () {
    ...
    function getBuildingSection(row) {
        ...
        return map ? map[0] : 'غير مصنف';
    }

    function getBuildingSort(row) {
        let map = getBuildingMap(row);
        return map ? map[1] : 999999;
    }
...
var KTBuildingUnitsList = function () {
    ...
    var initTable = function () {
        datatable = $(table).DataTable({
            ...
            columns: [
                { data: 'housing_unit_type', name: 'housing_unit_type', className: 'text-start px-6 py-4' },
                { data: 'unit_damage_status', name: 'unit_damage_status', className: 'text-center px-6 py-4' },
                { data: 'floor_number', name: 'floor_number', className: 'text-center px-6 py-4' },
                { data: 'housing_unit_number', name: 'housing_unit_number', className: 'text-center px-6 py-4' },
                { data: 'owner_name', name: 'owner_name', className: 'text-start px-6 py-4 min-w-280px' },
                { data: 'unit_direction', name: 'unit_direction', className: 'text-center px-6 py-4' },
                { data: 'legal_audit_status', name: 'legal_audit_status', className: 'text-center px-6 py-4' },
                { data: 'engineering_audit_status', name: 'engineering_audit_status', className: 'text-center px-6 py-4' },
                { data: 'final_approval_status', name: 'final_approval_status', className: 'text-center px-6 py-4' }
            ],
