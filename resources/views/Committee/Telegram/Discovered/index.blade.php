...
@section('title', 'Telegram Discovered Chats')
@section('pageName', 'Telegram Discovered Chats')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            $('#telegram_discovered_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route('telegram.discovered.data') }}',
                order: [[6, 'desc']],
                columns: [
            ...
            $(document).on('click', '.promote-discovered-chat', function () {
                $('#promote_discovered_chat_form').attr('action', '{{ url('telegram/discovered-chats') }}/' + $(this).data('chat-id') + '/promote');
