...
test('housing audit detail card fills the row beside the summary card', function () {
    ...
    expect($view)
        ...
        ->toContain('#tab_housing .housing-summary-card.is-fixed')
        ->toContain('#tab_housing .housing-summary-card.is-anchored-bottom')
        ->toContain('align-items: stretch')
        ->toContain('position: relative')
        ->toContain('top: 88px')
        ->toContain('max-height: calc(100vh - 104px) !important')
        ->toContain('function updateHousingSummaryPosition')
        ->toContain("document.querySelector('#tab_housing #housing_table')?.closest('.card')")
        ->toContain('unitsTableCard.getBoundingClientRect().bottom > topOffset + 16')
        ->toContain("window.addEventListener('scroll', updateHousingSummaryPosition")
        ->toContain("card.classList.add('is-fixed')")
